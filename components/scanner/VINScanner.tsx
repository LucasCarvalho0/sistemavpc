...
// @ts-ignore - Quagga doesn't have good types sometimes
...
/**
 * SCANNER VIN V10 DUAL-ENGINE PRO
 * - Motores: ZXing (Multi) + QuaggaJS (Industrial 1D/Code128)
 * - Resolução: 1080p forced para precisão milimétrica
 * - HUD: Industrial Vision HUD com Retículo Dinâmico
 * - Inteligência: Normalização ISO 3779 v3 (O->0, I->1, Q->0)
 */
export default function VINScanner({ onScan, onClose }: Props) {
  ...
  const [engine, setEngine] = useState<'both' | 'zxing' | 'quagga'>('both')
  ...
  useEffect(() => {
    async function startCamera() {
      try {
        ...
        // Inicia os dois motores simultâneos
        startZXing()
        startQuagga()
    ...
    <div className="scanner-overlay fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-0 inset-x-0 p-6 flex items-center justify-between bg-gradient-to-b from-black/95 to-transparent z-20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h2 className="text-white font-black text-sm uppercase tracking-[0.2em] leading-none mb-1">Industrial V10</h2>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[9px] text-green-400 font-black tracking-widest uppercase">Dual Engine Active</span>
            </div>
          </div>
        </div>
      ...
      {status === 'error' ? (
        ...
        <div className="relative w-full h-full flex flex-col items-center justify-center p-0">
          ...
          {/* Mira Pro (Dual Engine Area) */}
          <div className="relative z-10 w-[92%] h-[160px] rounded-[2rem] border-2 border-blue-500/40 shadow-[0_0_0_9999px_rgba(0,0,0,0.6)] backdrop-blur-[1px]">
            {/* Linha Laser Scan */}
            <div className="absolute inset-x-8 h-[2px] bg-red-600 shadow-[0_0_25px_rgba(220,38,38,1)] animate-laser-scan" />

            {/* Cantos de Foco High Contrast */}
            <div className="absolute -top-1 -left-1 w-14 h-14 border-t-4 border-l-4 border-white rounded-tl-[2rem]" />
            <div className="absolute -top-1 -right-1 w-14 h-14 border-t-4 border-r-4 border-white rounded-tr-[2rem]" />
            <div className="absolute -bottom-1 -left-1 w-14 h-14 border-b-4 border-l-4 border-white rounded-bl-[2rem]" />
            <div className="absolute -bottom-1 -right-1 w-14 h-14 border-b-4 border-r-4 border-white rounded-br-[2rem]" />
            ...
            <div className="absolute -bottom-12 inset-x-0 text-center">
              <div className="inline-flex items-center gap-2 bg-black/60 px-4 py-1.5 rounded-full border border-white/10 backdrop-blur-lg">
                <Cpu className="w-3 h-3 text-blue-400" />
                <span className="text-[10px] text-white/80 font-black uppercase tracking-[0.2em]">Proc: ZXing + Quagga</span>
              </div>
            </div>
          </div>
          ...
          <div className="absolute bottom-28 inset-x-0 text-center px-10 space-y-3 z-20 pointer-events-none">
            <h3 className="text-white font-black text-2xl uppercase italic tracking-tighter">Escaneamento Profissional</h3>
            <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest leading-relaxed max-w-[300px] mx-auto">
              Aproxime o código de barras até preencher a mira. O sistema detecta VIN instantaneamente.
            </p>

            {/* Terminal de Diagnóstico */}
            {(debugMode || lastScan) && (
              <div className="mt-8 p-4 bg-black/90 border border-white/10 rounded-2xl backdrop-blur-2xl animate-in slide-in-from-bottom-5">
                <div className="flex items-center gap-2 mb-2">
                  <Terminal className="w-3 h-3 text-blue-400" />
                  <span className="text-[9px] text-blue-400 font-black uppercase tracking-widest">Vision Lab V10</span>
                </div>
                <div className="space-y-2 text-left font-mono">
                  <div className="bg-white/5 p-2 rounded-lg flex justify-between items-center">
                    ...
                    <span className="text-xs text-green-400 font-bold tracking-wider">{lastScan || 'READY'}</span>
                  </div>
                  {debugMode && (
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-white/5 p-2 rounded-lg">
                        <span className="text-[8px] text-slate-500 block uppercase font-bold">Res</span>
                        <span className="text-[10px] text-white font-bold">1080p FHD</span>
                      </div>
                      <div className="bg-white/5 p-2 rounded-lg">
                        <span className="text-[8px] text-slate-500 block uppercase font-bold">Focus</span>
                        <span className="text-[10px] text-white font-bold">Auto Cont.</span>
                      </div>
                    </div>
                  )}
                </div>
